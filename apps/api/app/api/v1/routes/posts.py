...
# =========================================================
# ­ƒôä LIST POSTS WITH PAGINATION
# =========================================================
...
def list_saved_posts(
    ...
    ids = [row[0] for row in saved_rows]
    if not ids:
        return []
    posts = post_query(db).filter(Post.id.in_(ids)).all()
    posts_by_id = {p.id: p for p in posts}
    ordered = [posts_by_id[i] for i in ids if i in posts_by_id]
    return [to_post_response(db, p, current_user) for p in ordered]
