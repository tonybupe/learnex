...
export default function HomePage() {
  ...
  const miniStats = isTeacher ? [
    { label: "Classes",  value: stats?.total_classes  ?? "—", color: "#cb26e4" },
    { label: "Students", value: stats?.total_learners ?? "—", color: "#38bdf8" },
    { label: "Lessons",  value: stats?.total_lessons  ?? "—", color: "#22c55e" },
  ] : isAdmin ? [
    { label: "Users",   value: stats?.total_users   ?? "—", color: "#ef4444" },
    { label: "Classes", value: stats?.total_classes ?? "—", color: "#cb26e4" },
    { label: "Lessons", value: stats?.total_lessons ?? "—", color: "#38bdf8" },
  ] : [
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "20px 16px 48px" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 20, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {miniStats.map(s => (
              <div key={s.label} style={{ padding: "10px 16px", borderRadius: 12, background: "var(--card)", border: "1px solid var(--border)", textAlign: "center", minWidth: 72 }}>
                <div style={{ fontSize: 20, fontWeight: 900, color: s.color, lineHeight: 1 }}>{s.value}</div>
                <div style={{ fontSize: 10, color: "var(--muted)", fontWeight: 600, marginTop: 3 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
